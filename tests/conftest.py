...
def hermetic_settings(tmp_path, monkeypatch):
    ...
    * `vm/config.py` now loads `<repo>/.env` into os.environ. A suite that reads it would pass or
      fail according to whatever the developer last persisted, so VM_ENV_FILE is pointed at a
      path that does not exist.
    * `load_env_file` mutates os.environ directly, so a test that triggers a load leaves VM_*
      variables set for every test that follows. Snapshot and restore them.
    """
    monkeypatch.setenv("VM_ENV_FILE", str(tmp_path / "no-such.env"))
    before = {k: v for k, v in os.environ.items() if k.startswith("VM_")}
    config._LOAD_REPORT = {}
    yield
    for key in [k for k in os.environ if k.startswith("VM_")]:
