#!/usr/bin/env python
"""The one entry point every `vm` shim executes. Not meant to be typed by a human.

WHY THIS FILE EXISTS. The shims used to invoke the CLI as an inline program:

    python -c "import sys; sys.path.insert(0, r'$VM_ROOT'); from vm.cli import main; ..."

which fails in two ways that are only visible on Windows. First, `$VM_ROOT` had to be spliced
...
# Put the repo root FIRST so `vm` resolves to this checkout even if another one is installed
# in the interpreter's site-packages — a shim named after a directory must mean that directory.
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from vm.cli import main  # noqa: E402
