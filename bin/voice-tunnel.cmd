@echo off
REM PATH shim for cmd.exe and PowerShell. Same contract as bin/voice-tunnel: identical behaviour from
...
for %%I in ("%~dp0..") do set "VOICE_TUNNEL_ROOT=%%~fI"
...
if not exist "%VOICE_TUNNEL_ROOT%\bin\voice-tunnel-run.py" (
  echo {"error": "voice-tunnel shim cannot find its repo", "code": "detached_shim", "remedy": "keep this file inside the checkout's bin directory and put that directory on PATH; do not copy it elsewhere"} 1>&2
  exit /b 2
)
...
if exist "%VOICE_TUNNEL_ROOT%\venv\Scripts\python.exe" (
  set "VOICE_TUNNEL_PY=%VOICE_TUNNEL_ROOT%\venv\Scripts\python.exe"
) else (
...
"%VOICE_TUNNEL_PY%" "%VOICE_TUNNEL_ROOT%\bin\voice-tunnel-run.py" %*
exit /b %ERRORLEVEL%
